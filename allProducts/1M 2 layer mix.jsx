...
{ ////////////////////////////////////////////////////////////// chay lop mica
    ...
    for (var i = stt; i <= arr.length - 1; i++) {
        ...
        var soLayerCut = [[[1, 3, 1, 4], [1, 4, 1, 4]]];

        if ((arr[i].nameId).substr(3, 2) == "WA") soLayerCut = [[[1, 1, 1, 4], [1, 2, 1, 4]]];
...
{ ////////////////////////////////////////////////////////////// chay lop go
    ...
    for (var i = stt; i <= arr.length - 1; i++) {
        #include "convertPixel.jsx";
        var StatusCanGiua = false; // trạng thái sau khi duplicate có căn giữa với nhau không
        var soLayerCut = [[[1, 1, 1, 4], [1, 2, 1, 4]]];

        if ((arr[i].nameId).substr(3, 2) == "WA") soLayerCut = [[[1, 3, 1, 4], [1, 4, 1, 4]]];
...
function tinhkichthuoc(item, FileDesign, type) {
    ...
    var activeHeight = app.activeDocument.height;
    cropBoxInXY_onlyDuplicate(1, 1, 1, 4, app.activeDocument.width, app.activeDocument.height);

    app.activeDocument.activeLayer = app.activeDocument.layers[1];

    cropBoxInXY_onlyDuplicate(1, 3, 1, 4, app.activeDocument.width, app.activeDocument.height);
    ...
    if (bounds1[0] > bounds2[0]) b0 = bounds2[0];
    if (bounds1[1] > (bounds2[1] - activeHeight / 2)) b1 = bounds2[1] - (activeHeight / 2);
    if (bounds1[2] < bounds2[2]) b2 = bounds2[2];
    if (bounds1[3] < (bounds2[3] - activeHeight / 2)) b3 = bounds2[3] - (activeHeight / 2);
